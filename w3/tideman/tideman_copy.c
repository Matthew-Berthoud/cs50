...
void lock_pairs(void)
{
    locked[pairs[0].winner][pairs[0].loser] = true; // first one always locks
    for (int i = 1; i < pair_count; i++) // for each subsequent pair
    {
        int winner = pairs[i].winner;
        int loser = pairs[i].loser;
        if (find_source(winner) == loser) // ADDING THIS WOULD CYCLE
        {
            continue;
        }
...
// bool cycle_check(int winner, int loser) // recursively check for cycle
// {
//     for (int i = 0; i < candidate_count; i++)
//     {
//         if (locked[i][winner]) // something is beating this winner
//         {
//             if (i == loser) // CYCLE ALERT
//             {
//                 return false;
//             }
//             return cycle_check(i, winner); // no cycle yet, but keep going
//         }
//     }
//     return true;
// }
